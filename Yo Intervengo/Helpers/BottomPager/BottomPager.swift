...
class BottomPager:  UIView,UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    init(frame: CGRect, array:Array<Report>) {
        ...
        let v1 = UIView(frame: CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height))
    ...
    func scrollViewDidEndDecelerating(scrollView: UIScrollView) {
        let visibleRect = CGRect(x: self.collectionView.contentOffset.x, y: self.collectionView.contentOffset.y, width: self.collectionView.frame.width, height: self.collectionView.frame.height)
