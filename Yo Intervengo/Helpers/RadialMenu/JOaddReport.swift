    ...
    class JOaddReport: UIView,LNERadialMenuDataSource,LNERadialMenuDelegate,JOSideBarMenuDelegate,JOCentralMenuDelegate,JSImagePickerViewControllerDelegate,UITextFieldDelegate, UITextViewDelegate, APIManagerDelegate{
        ...
        init(frame: CGRect, bttnClose:UIButton, labels:Int, coodinate:CLLocationCoordinate2D) {
            ...
            let blurView = UIView(frame: CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height))
            ...
            btnBack.addTarget(self, action: #selector(JOaddReport.goBack(_:)), forControlEvents: UIControlEvents.TouchUpInside)
            ...
            btnContinue.addTarget(self, action: #selector(JOaddReport.goContinue(_:)), forControlEvents: UIControlEvents.TouchUpInside)
        ...
        func showMenu(step:Int, atPoint point:CGPoint){
            ...
            case 1:
                ...
                let thisMenu = LNERadialMenu(fromPoint: point, withDataSource: self, andDelegate: self,withFrame: self.frame, andLabels:Bool(labels))
                thisMenu.radialMenuIdentifier = "show"
                self.insertSubview(thisMenu, belowSubview: btnClose)
                thisMenu.showMenu()
            case 2:
                conn._getCategories({[unowned self] (data, error) in
                    print(data)
                    self.sideData = self.conn.getCategories(self.lblIndicator.text == "REPORTE")
                    self.this = JOSideBarMenu(frame: CGRectMake(0, 0, 320, min(370, self.frame.height - (100 + self.btnClose.frame.height)))  , data: data)
                    self.this.frame.origin = CGPoint(x: 0, y: (self.frame.height - self.btnClose.frame.height)-self.this.frame.height)
                    self.this.delegate = self
                    self.addSubview(self.this)
                    })
            case 3:
                _ = NSMutableArray()
            ...
            case 5:
                ...
                let placeholder = NSAttributedString(string: "Titulo", attributes: [NSForegroundColorAttributeName : UIColor.lightGrayColor()])
                ...
                let tit = UILabel(frame: CGRect(x: 0, y: 10, width: txtDesc.frame.width, height: 15))
            ...
            case 6:
                if txtTitle.text == "" || txtD.text == "" {
                    ...
                    btnAddImage.addTarget(self, action: #selector(JOaddReport.openCamera(_:)) , forControlEvents: UIControlEvents.TouchUpInside)
        ...
        func radialMenu(radialMenu: LNERadialMenu!, closingMenu close: Bool) {
            //closeMenu()
        }
        ...
        func radialMenu(radialMenu: LNERadialMenu!, elementAtIndex index: Int) -> UIButton! {
            let element : UIButton = UIButton(frame: CGRect(x: 0, y: 0, width: 70, height: 70))
            switch index{
        ...
        func viewForCenterOfRadialMenu(radialMenu: LNERadialMenu!) -> UIView! {
            let centerView:UIView = UIView(frame: CGRect(x: 0, y: 0, width: 190, height: 190))
        ...
        func buttoTapped(button:UIImageView!,withSideBar sideBar:JOSideBarMenu,label:String , id:Int){
            ...
            btnSubcategory.addTarget(self, action: #selector(JOaddReport.goBack(_:)), forControlEvents: UIControlEvents.TouchUpInside)
            ...
            let pop2 = POPSpringAnimation(propertyNamed: kPOPViewCenter)
            ...
            let pop3 = POPSpringAnimation(propertyNamed: kPOPViewSize)
