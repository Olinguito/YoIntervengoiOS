    ...
    class DetailReportVC: GenericViewController,UIScrollViewDelegate,JOTabBarDelegate,UIGestureRecognizerDelegate,JSImagePickerViewControllerDelegate {
        ...
        func tappedButton(){
            if (buttonHelper?.isDescendantOfView(self.view) != nil){
                buttonHelper.removeTarget(self, action: #selector(DetailReportVC.openView(_:)), forControlEvents: UIControlEvents.TouchUpInside)
            ...
            if tab.getActHelper().tag > 0{
                ...
                case 2: buttonHelper.addTarget(self, action: #selector(DetailReportVC.openView(_:)), forControlEvents: UIControlEvents.TouchUpInside)
                buttonHelper.setImage(UIImage(named: "linkHelper"), forState: UIControlState.Normal)
                case 1: buttonHelper.addTarget(self, action: #selector(DetailReportVC.openView(_:)), forControlEvents: UIControlEvents.TouchUpInside)
                buttonHelper.setImage(UIImage(named: "btnNewPicture"), forState: UIControlState.Normal)
                default: print("")
                }
        ...
        func openView(sender:UIButton){
            ...
            case 1: let imagePicker = JSImagePickerViewController()
            imagePicker.delegate = self
            imagePicker.showImagePickerInController(self, animated: true)
            case 2: let r = CreateReport(frame: self.view.frame, bttnClose: buttonHelper)
        ...
        override func returnObt(responseObject: AnyObject, url: String) {
            var response = JSON(responseObject)
            switch(url){
            case "report_detail":
                let dataDetail = JSON(responseObject)
                ...
                btnBack.addTarget(self, action: #selector(UIWebView.goBack), forControlEvents: UIControlEvents.TouchUpInside)
                ...
                btnInfo.addTarget(self, action: #selector(DetailReportVC.goReport(_:)), forControlEvents: UIControlEvents.TouchUpInside)
